...
class SaleWizard(models.TransientModel):
    ...
    _description = 'Wizard: Quick SOs for Session Students'
    _transient = True
    
    def _default_session(self):
        # get current active id from context, browse through session env
    ...
    session_student_ids = fields.Many2many(comodel_name='res.partner',
                                           string='Students in Session',
                                           related='session_id.student_ids',
                                           help='Students that are currently in session')
    
    student_ids = fields.Many2many(comodel_name='res.partner',
                                   string='Students for SOs')
